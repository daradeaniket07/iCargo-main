...
export default function OPR026({ awb }: { awb: string }) {
  ...
  return (
    <div className="w-full bg-white border rounded-md shadow-sm mt-10 p-3 sm:p-4 lg:p-6">
      ...
      <div className="px-2 sm:px-3 pt-2">
        <nav className="flex flex-wrap gap-2 text-xs sm:text-sm">
          <button
            onClick={() => setActiveTab("general")}
            className={`px-3 py-2 border rounded-t ${
              activeTab === "general" ? "text-blue-600 font-semibold" : "text-gray-600"
            }`}
          >
            General
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-3">
          <Label>Routing *</Label>
          <Input value={data.routing} />
        </div>

        <div className="lg:col-span-3">
          <Label>Requested Flight</Label>
          <Input value={data.requestedFlight} />
        </div>
      ...
      <div className="border rounded p-3 bg-gray-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-2">
            <Label>Code</Label>
            <Input value={data.agentCode} />
          </div>

          <div className="lg:col-span-5">
            <Label>Name</Label>
            <Input value={data.agentName} />
          </div>

          <div className="lg:col-span-2">
            <Label>IATA Code</Label>
            <Input value={data.iataCode} />
          </div>

          <div className="lg:col-span-2">
            <Label>CASS Code</Label>
            <Input value={data.cassCode} />
          </div>

          <div className="lg:col-span-1 flex items-end">
            <button className="px-3 py-1 border rounded text-xs sm:text-sm w-full">
              Tax Info
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        {/* SHIPPER */}
        <div className="lg:col-span-6 border rounded p-3 bg-white">
          <div className="text-sm font-semibold mb-2">SHIPPER / CONSIGNEE DETAILS</div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <ActionButton text="Capture Irregularity" />
            <ActionButton text="Split Shipment" />
            <ActionButton text="HAWB" />
        ...
        <div className="lg:col-span-6 border rounded p-3 bg-white">
          ...
          <div className="mt-3 flex flex-wrap justify-end gap-2">
            <BlueButton text="View/Upload Files" />
            <GrayButton text="Print" />
...
/* ---------------------------------------------------
                CHARGES TAB (Responsive)
--------------------------------------------------- */
function ChargesTab() {
  return (
    <div className="px-1 sm:px-2 text-xs sm:text-sm">
      ...
      <div className="border p-3 rounded bg-white shadow-sm mb-5">
        ...
        {/* THREE GRIDS */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-12 gap-4 mb-3">
          <InputBox label="Currency" value="INR" />
          <InputBox label="Charge Code" value="PP" />
          <InputBox label="Payment Type *" value="PP" />
          <InputBox label="Insurance Amt" value="0.00" />
        ...
        {/* SECOND ROW */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 mb-3">
          <div className="lg:col-span-3">
            <Label>Rated Customer *</Label>
            <Input value="NIKKDELIN" />
          </div>
      ...
      {/* RATING TABLE (scrollable) */}
      <div className="font-semibold">RATING DETAILS</div>

      <div className="overflow-auto border rounded mt-2 bg-white text-xs">
        <table className="min-w-[1200px] border-collapse">
          <thead className="bg-gray-100">
            <tr>
              {[
                "No of Pcs","Weight","Adjusted Weight","RCP","Rate Class","Commodity",
                "IATA Code","Service Code","Chargeable Weight","IATA Rate","IATA Charge",
                "Volume","Country of Origin","ULD","Description","Rate/Pivot","Net Charge"
              ].map((h,i)=>(<th key={i} className="border px-2 py-1">{h}</th>))}
            </tr>
          </thead>
          ...
          <tbody>
            <tr>
              {[
                "8","181","0","Q","GEN","-","-","-","230","330",
                "75,900.00","1.38","-","NO","SAID TO CONTAIN","226.95","52,198.50"
              ].map((v,i)=>(<td key={i} className="border px-2 py-1">{v}</td>))}
            </tr>
          </tbody>
        </table>
      </div>
      ...
      {/* CHARGE DETAILS */}
      <div className="mt-6 font-semibold">CHARGE DETAILS</div>
      ...
      <div className="border rounded p-3 bg-white mt-2">
        ...
        <div className="overflow-auto">
          <table className="min-w-[700px] text-xs border-collapse border">
            <thead className="bg-gray-100">
              <tr>
                {["", "Code","Charge Head Name","Charge","PP/CC","Due Carrier","Due Agent","Remarks"]
                .map((h,i)=><th key={i} className="border px-2 py-1">{h}</th>)}
      ...
      {/* ACCOUNTING SUMMARY */}
      <div className="mt-6 flex justify-end">
        <div className="border rounded p-3 bg-white text-xs w-full sm:w-[350px] shadow-sm">
...
const Input = ({ value }: any) => (
  <input className="w-full border rounded px-2 py-1" value={value} readOnly />
);

const ActionButton = ({ text }: any) => (
  <button className="px-3 py-1 border rounded text-xs sm:text-sm bg-gray-100">
    {text}
  </button>
);

const BlueButton = ({ text }: any) => (
  <button className="px-3 py-1 border rounded text-xs sm:text-sm bg-blue-600 text-white">
    {text}
  </button>
);

const GrayButton = ({ text }: any) => (
  <button className="px-3 py-1 border rounded text-xs sm:text-sm bg-gray-200">
    {text}
  </button>
);
